...
class CAMBparams(CAMB_Structure):
    ...
    def __init__(self):
        ...
        # MGCAMB MOD START
        self.mgcamb_flags.set_params()
        self.mgcamb_model_pars.set_params()
        self.mgcamb_par_cache.set_params()
        # MGCAMB MOD END
    ...
    # MGCAMB MOD START: setting the parameters
    def set_mgcamb(self, flags, model_pars, pars_cache):
        """
           Set all the MGCAMB parameters
        """
    
        assert (isinstance(flags, mgcamb.MGCAMBFlags))
        assert (isinstance(model_pars, mgcamb.MGCAMBModelParams))
        assert (isinstance(pars_cache, MGCAMB_parameter_cache))
    
        MGCAMB_setparams(byref(self), byref(flags), byref(model_pars), byref(pars_cache))
        return self
    # MGCAMB MOD END
