...
!----
!ALEX added wnu_arr, grhodot_pi
    subroutine MassiveNuVarsOut(EV,y,yprime,a,grho,gpres,dgrho,dgq,dgpi, gdpi_diff,pidot_sum,clxnu_all, grhodot_pi)
    implicit none
    type(EvolutionVars) EV
    real(dl) :: y(EV%nvar), yprime(EV%nvar),a 
    real(dl), optional :: grho,gpres,dgrho,dgq,dgpi, gdpi_diff,pidot_sum,clxnu_all, grhodot_pi
real(dl) :: wnu_arr(CP%Nu_mass_eigenstates)
    !grho = a^2 kappa rho
    !gpres = a^2 kappa p
...
!grhodot_pi = sum dot(a^2 kappa rho) pi
...
!---
! ALEX 
if (present(grhodot_pi)) then
wnu_arr(nu_i) = pnu/rhonu
!write(*,*) wnu_arr(nu_i)
grhodot_pi = grhodot_pi - (1.d0 + 3.d0*wnu_arr(nu_i))* grhonu_t
end if
...
! For neutrinos compatibility
real(dl) grhodot_pi        !grhodot_pi = \sum dot(kappa a^2 rho) pi
!* MGCAMB mod: end
...
grhodot_pi = 0
    ...
    if (CP%Num_Nu_Massive /= 0) then
        call MassiveNuVarsOut(EV,y,yprime,a,grho,gpres,dgrho,dgq,dgpi, dgpi_diff,pidot_sum, grhodot_pi=grhodot_pi)
    end if
!----
!---
! TEST
!if (dgpi /=0 .or. dgpi_diff /=0 .or. pidot_sum /= 0) then
!   write(*,*) dgpi, dgpi_diff, pidot_sum
!else if
...
!********************************
 ...
 qgdot =yprime(EV%g_ix+1) !ISITGR uses different qgdot here...
end if ! tempmodel /= 0
...
!----
grhodot_pi = grhodot_pi -2.d0 * grhor_t * pir - 2.d0* grhog_t * pig
grhodot_pi = adotoa * grhodot_pi
!----
...
!****************************************************
 ...
 !*************************************************************
    ...
    !ALEX adding term 0 for MG_rhoDeltadot
...
!-(MG_mudot*MG_gamma+MG_mu*MG_gammadot)*2.d0*dgpi/k2 )/MG_gamma
!-----
!remember that I added a factor 2 in front of pidot_sum
!if(model==1 .or. model==4 .or. model==5.or. model==6 .or. model == 7 .or. model ==8 .or. model == 9 .or. model ==10) MG_psidot = &
!(MG_phidot - MG_gammadot * MG_psi -MG_mu/k2 * (pidot_sum - adotoa*(2.d0*dgpi + dgpi_diff)) - MG_mudot *dgpi/k2 ) /MG_gamma!- &! added it...
!MG_mu * 3.d0 * dgpi * adotoa / k2 )/ MG_gamma
!if(model==1 .or. model==4 .or. model==5.or. model==6 .or. model == 7 .or. model ==8 .or. model == 9 .or. model ==10) MG_psidot = &
!(MG_phidot - MG_gammadot * MG_psi -MG_mu/k2 * (diff_rhopi)- MG_mudot *dgpi/k2)/MG_gamma!- &! added it...

!- 2.d0* adotoa*(2.d0*dgpi + dgpi_diff)

if(model==1 .or. model==4 .or. model==5.or. model==6 .or. model == 7 .or. model ==8 .or. model == 9 .or. model ==10) ISW_MG = &
- (MG_gammadot* MG_mu + MG_gamma* MG_mudot)*0.5d0/k2 * (dgrho + 2.d0*dgpi) - &
MG_mu* MG_gamma*0.5d0/k2*(MG_rhoDeltadot + 2.d0* dgpidot) - MG_mudot*0.5d0/k2*dgrho - MG_mu*0.5d0/k2*MG_rhoDeltadot
...
!-----
!modified below..
	    ISW_MG= expmmu(j) * ISW_MG!* (MG_phidot + MGq_psidot)
	    ISW=ISW_MG

!----
!ISW = (4.D0/3.D0*k*sigma+(-2.D0/3.D0*sigma-2.D0/3.D0*etak/adotoa)*k &
!-diff_rhopi/k**2-1.D0/adotoa*dgrho/3.D0+(3.D0*gpres+5.D0*grho)*sigma/k/3.D0 &
!-2.D0/k*adotoa/EV%Kf(1)*etak)*expmmu(j)
!----
...
!ALEX test
    if (CP%Num_Nu_Massive > 0) then
        ...
        !call MassiveNuVarsOut(EV,ay,ayprime,a,dgpi=dgpi) ! Added this...
    end if
...
!==========
! WARNING!!
! there's no modification here!!! WHY???\
!
! Mod ADDED!
!
!==========
...
!ALEX test (compatibility with massive neutrinos)
dgpi = 0
...
!write(*,*) "mu, gamma and derivs"
!write(*,*) MG_mu, MG_gamma, MG_mudot, MG_gammadot
